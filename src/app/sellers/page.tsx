...
const SELLER_TYPES = ["Farmer", "Trader", "Wholesaler", "Agent", "Other"];

const EMPTY_FORM = { name: "", phone: "", address: "", seller_type: "Farmer" };
...
function Drawer({ open, onClose, title, children }: any) {
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-lg bg-white shadow-2xl z-50 flex flex-col overflow-hidden transform transition-transform">
...
export default function SellersRegistryPage() {
  ...
  const fetchSellers = async () => {
    setIsFetching(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/sellers?page=${page}&limit=12&search=${search}`);
      const data = await res.json();
      if (data.items) { setSellers(data.items); setMeta(data.meta); }
  ...
  const openEdit = (seller: any) => {
    setForm({ name: seller.name, phone: seller.phone || "", address: seller.address || "", seller_type: seller.seller_type || "Farmer" });
    setError("");
    setDrawerMode("edit");
  ...
  const handleSave = async () => {
    ...
    setError("");
    try {
      const url = drawerMode === "add" ? `${API_BASE_URL}/api/sellers` : `${API_BASE_URL}/api/sellers/${selectedId}`;
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/sellers/${id}`, { method: "DELETE" });
      fetchSellers();
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.4em]">Sellers</span>
          </div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter flex items-center gap-3">
            <Users size={32} /> Farmers & Sellers
          </h1>
          <p className="text-slate-500 font-medium mt-1 text-sm">Manage all farmers and supplier partner details.</p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl shadow-slate-200/40 overflow-hidden">
        ...
        <div className="p-8 border-b border-slate-100 flex gap-4 bg-slate-50/30">
          <div className="relative flex-1 group max-w-xl">
            ...
            <input type="text" placeholder="Search by Name, Phone, or Address..." value={search}
          ...
          <button onClick={fetchSellers} className="p-3.5 bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-indigo-600 transition-all shadow-sm">
            <TrendingUp size={18} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 p-10">
          {isFetching ? Array(6).fill(0).map((_, i) => (
            ...
            <div key={seller.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl shadow-slate-200/30 group hover:border-indigo-200 transition-all relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-8 relative z-10">
                <div className="w-14 h-14 bg-slate-50 border border-slate-100 text-slate-400 group-hover:bg-indigo-600 group-hover:text-white group-hover:rotate-6 rounded-2xl flex items-center justify-center transition-all shadow-sm">
                  <Briefcase size={28} />
                </div>
                <span className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                  seller.seller_type === 'Farmer' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                }`}>
                  <ShieldCheck size={12} /> {seller.seller_type || "Farmer"}
                </span>
              </div>
              <div className="space-y-1 mb-6 relative z-10">
                <h3 className="text-xl font-black text-slate-900 tracking-tighter uppercase truncate">{seller.name}</h3>
                <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Seller #{seller.id.toString().padStart(4, '0')}</span>
              </div>
              <div className="space-y-3 relative z-10">
                {seller.phone && <div className="flex items-center gap-3 text-slate-500"><Phone size={14} className="text-indigo-400" /><span className="text-[10px] font-black uppercase tracking-tight">{seller.phone}</span></div>}
                {seller.address && <div className="flex items-start gap-3 text-slate-500"><MapPin size={14} className="text-indigo-400 mt-0.5 shrink-0" /><span className="text-[10px] font-bold uppercase tracking-tight line-clamp-2 leading-relaxed">{seller.address}</span></div>}
              </div>
              <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-end gap-2 relative z-10">
                <button onClick={() => openEdit(seller)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all border border-transparent hover:border-indigo-100">
                  <Edit3 size={16} />
                </button>
                <button onClick={() => handleDelete(seller.id, seller.name)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all border border-transparent hover:border-red-100">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
        ...
        {!isFetching && sellers.length === 0 && (
          <div className="p-32 text-center flex flex-col items-center gap-6">
            ...
            <p className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Partner Network Empty</p>
          </div>
        )}

        {/* Pagination */}
        <div className="p-8 bg-slate-50/50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Showing {sellers.length} of {meta.totalItems} Partners</p>
      ...
      {/* Add / Edit Drawer */}
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} title={drawerMode === "add" ? "Onboard New Partner" : "Edit Partner"}>
        <div className="space-y-6">
          <Field label="Full Name *" icon={User} type="text" placeholder="Seller / Farmer Name" value={form.name} onChange={(e: any) => setForm({ ...form, name: e.target.value })} />
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] pl-1">Seller Type</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] pl-1">Address</label>
            <div className="relative group">
              ...
              <textarea rows={3} placeholder="Village, District, State" value={form.address}
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button onClick={handleSave} disabled={saving} className="flex-1 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-xl shadow-indigo-600/30 disabled:opacity-50">
              <Save size={16} /> {saving ? "Saving..." : drawerMode === "add" ? "Create Partner" : "Save Changes"}
            </button>
          </div>
        </div>
      </Drawer>
